...
public class AddIngredients extends AppCompatActivity {
   ...
   // private ImageButton plusIc; //Add button that takes the string from addIngred to entN, and creates new row
    ...
    public void getValues(View v){
        ...
        int counter = 0;
        for (int i = 2; i < c-1; i++){
            View r = mContainerView.getChildAt(i); //get these into a string
            //Log.i("ViewName", r.getClass().getName());
            String val = ((EditText)r.findViewById(R.id.editText)).getText().toString();//this is coming out null
            if(counter == 0)
                send = val;
            else
        ...
        keywords = send;
        client myClient = new client("192.168.0.15", 9090);
        myClient.execute();
    }
...
// Inflate at the end of all rows but before the "Add new" button
    ...
    public class client extends AsyncTask<Void, Void, String> {
        ...
        protected void onPostExecute(String result) {
            Intent intent = new Intent(AddIngredients.this, RecipeList.class);
            intent.putExtra("recipe_package", result);
